...
if ! command -v nix >/dev/null 2>&1; then
  echo "error: nix not found; run ci_install_nix.sh first" >&2
  exit 1
fi

# Pin a nixpkgs that provides llvmPackages_23 (CI must not float on channels).
# Independent of the main flake chipcompiler nixpkgs pin (which tops out at LLVM 21).
# Resolved from nixos-unstable when llvmPackages_23.llvm.name == llvm-23.1.0.
