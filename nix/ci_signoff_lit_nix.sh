...
inner="$ECC_REPO_ROOT/nix/ci_signoff_lit_nix_inner.sh"

echo "nix shell ${NIXPKGS_REF}#lit + #llvmPackages_23.libllvm + #jq → ci_signoff_lit.sh"
# FileCheck from llvmPackages_23.libllvm; lit from nixpkgs lit.
exec nix shell \
  --extra-experimental-features 'nix-command flakes' \
  "${NIXPKGS_REF}#lit" \
  "${NIXPKGS_REF}#llvmPackages_23.libllvm" \
  "${NIXPKGS_REF}#jq" \
  --command bash "$inner" "$@"
